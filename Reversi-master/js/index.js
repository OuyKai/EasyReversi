...
function getready() {
	...
	//console.log(document.firstChild.name)
    // 绘制棋盘
    drawChessboard();
    // 根据屏幕变化改变棋盘尺寸
    ...
    $('#chess-board').on('click touchend', function (e) {
        ...
        if (change_arr.length !== 0 && (flag <= 0 && upper_hand==true || ai_flag==false)) {
			message = target_row+""+target_column;
			if(ai_flag){
				socket.send(message)
...
function clearChessboard(){
	...
	var childNodes = chessboard.childNodes;
	for(var i = childNodes.length-1;i>=0;i--){
		chessboard.removeChild(childNodes[i]);
	}
...
function ai() {
	...
	step[1][2]=replacePos(step[1][2],1,ai_x);
	step[1][2]=replacePos(step[1][2],2,ai_y);
	//console.log(step[1][2]);
